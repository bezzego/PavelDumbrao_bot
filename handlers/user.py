...
async def cmd_collaboration(message: types.Message):
    ...
    await message.answer(
        ...
        "или в личные сообщения Павлу Думбрао: https://t.me/PavelDumbrao\n\n"
    )
...
async def callback_start_challenge(callback: types.CallbackQuery):
    ...
    story_text = (
        "✅ *Сделай сторис с фото/видео*, где ты показываешь процесс или результат челленджа.\n\n"
        "✅ Прикрепи в сторис свою реферальную ссылку.\n\n"
        "✅ Отправь скриншот этой сторис сюда в бота, чтобы получить *+50 баллов* на свой счёт!\n\n"
...
async def redeem_consultation_points_callback(callback: types.CallbackQuery):
    ...
    if user_data and user_data.get("points", 0) >= 800:
        ...
        await callback.message.answer(
            "📞 Поздравляем! Вы  консультацию с Павлом (1.5 ч).\n\n"
